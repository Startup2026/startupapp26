...
type TrendingJobsViewProps = {
  LayoutComponent: ComponentType<LayoutProps>;
  headline?: string;
  description?: string;
  actionLabel?: string;
  jobLinkBuilder: (jobId: string) => string;
};

export function TrendingJobsView({
  LayoutComponent,
  headline = "Trending Opportunities",
  description = "Discover the most popular jobs and internships based on student engagement and upcoming deadlines.",
  actionLabel = "Apply Now",
  jobLinkBuilder,
}: TrendingJobsViewProps) {
    ...
    const fetchTrendingJobs = async () => {
      ...
      try {
        ...
        if (res.success && Array.isArray(res.data)) {
          const recommendationData = res.data as TrendingJob[];
          if (recommendationData.length === 0) {
            setHasMore(false);
          } else {
            setJobs((prev) => {
              const newJobs: TrendingJob[] = page === 1 ? recommendationData : [...prev, ...recommendationData];
              return Array.from(new Map(newJobs.map((job) => [job._id, job])).values());
            });
          }
        } else {
          setHasMore(false);
    ...
    <LayoutComponent>
      <div className="p-6 lg:p-8 space-y-8 animate-fade-in">
        <div className="max-w-4xl mx-auto text-center space-y-4">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight lg:text-5xl">{headline}</h1>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center">
                      ...
                      <div className="p-6 flex items-center gap-3">
                        {job.hasApplied ? (
                          ...
                          <Link to={jobLinkBuilder(job._id)} className="w-full md:w-auto">
